...
def build_prompt(question: str, hits: list[dict]) -> str:
    ctx = "\n\n".join([f"Source: {h['title']} ({h['url']})\nExcerpt: {h['chunk']}" for h in hits])
...
def answer(
    ...
    if tool == "confluence_decision_rag":
        qv = _embed_model.encode([question], normalize_embeddings=True)[0].tolist()
    ...
    return {"answer": "No suitable tool found for this question.", "sources": []}
